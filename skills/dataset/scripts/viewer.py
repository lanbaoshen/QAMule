...
.step-success.ok { background: rgba(102,187,106,0.15); color: var(--green); border: 1px solid rgba(102,187,106,0.4); }
.step-success.fail { background: rgba(255,77,79,0.12); color: var(--accent); border: 1px solid rgba(255,77,79,0.3); }
.step-success.unknown { background: rgba(255,255,255,0.05); color: var(--text-muted); border: 1px solid var(--border); }
...
.step-body.side-by-side .thought {
  margin-bottom: 12px;
}

.step-body.side-by-side .screenshot-wrap {
  max-width: 100%;
  margin-top: 0;
}
...
<div class="app">
  <aside class="sidebar">
    <div class="sidebar-header">
      <h1>QAMule Trajectory Viewer</h1>
...
function renderTrajectory(traj, basePath) {
  ...
  const devW = traj.device?.resolution?.[0] || 1;
  const devH = traj.device?.resolution?.[1] || 1;
  ...
  let html = `<div class="traj-header">
    <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:12px">
      ...
      <div class="btn-group">
        <button class="btn-open" onclick="openFile('${basePath}')">&#128196; Open</button>
  ...
  for (const step of traj.steps) {
    ...
    if (step.success === true) {
      successBadge = `<span class="step-success ok">✓ success</span>`;
    } else if (step.success === false) {
      successBadge = `<span class="step-success fail">✗ failed</span>`;
    } else {
      successBadge = `<span class="step-success unknown">· pending</span>`;
    }
    ...
    if (step.screenshot) {
      ...
      if ((actionType === 'click' || actionType === 'long_click') && step.action.x != null) {
        ...
        const y = step.action.y / devH * 100;
        html += `<div class="click-marker" style="left:${x}%;top:${y}%"></div>`;
      }
      if (actionType === 'swipe' && step.action.x1 != null) {
        html += renderSwipeArrow(step.action.x1, step.action.y1, step.action.x2, step.action.y2, devW, devH);
      }
      html += `</div>`;
    }
...
function renderSwipeArrow(x1, y1, x2, y2, vw, vh) {
  const dx = x2 - x1, dy = y2 - y1;
  const len = Math.sqrt(dx * dx + dy * dy);
  if (len < 1) return '';

  const ux = dx / len, uy = dy / len;
  // Arrowhead dimensions (in device px); scale with swipe length but clamp
  const headLen = Math.min(len * 0.28, 90);
  const headWidth = headLen * 0.48;
  // Line stops just before the arrowhead base so it doesn't poke through
  const lx2 = x2 - ux * headLen * 0.62;
  const ly2 = y2 - uy * headLen * 0.62;
  ...
  // Stroke width adaptive to swipe length
  const sw = Math.max(8, Math.min(20, len * 0.022));
  const sr = sw * 1.4;  // start-dot radius

  return `<svg class="swipe-arrow" style="position:absolute;inset:0;width:100%;height:100%;"
    viewBox="0 0 ${vw} ${vh}" preserveAspectRatio="xMidYMid meet">
    <!-- dark shadow layer for contrast -->
    <line x1="${x1}" y1="${y1}" x2="${lx2}" y2="${ly2}"
      stroke="rgba(0,0,0,0.45)" stroke-width="${sw + 7}" stroke-linecap="round"/>
    <polygon points="${pts}" fill="rgba(0,0,0,0.45)"
      transform="translate(${(ux * 2.5).toFixed(1)},${(uy * 2.5).toFixed(1)})"/>
    <!-- coloured arrow -->
    <line x1="${x1}" y1="${y1}" x2="${lx2}" y2="${ly2}"
      stroke="#4fc3f7" stroke-width="${sw}" stroke-linecap="round" stroke-opacity="0.92"/>
    <polygon points="${pts}" fill="#4fc3f7" fill-opacity="0.92"/>
    <!-- start dot -->
    <circle cx="${x1}" cy="${y1}" r="${sr + 3}" fill="rgba(0,0,0,0.35)"/>
    <circle cx="${x1}" cy="${y1}" r="${sr}" fill="#4fc3f7" fill-opacity="0.6"/>
    <circle cx="${x1}" cy="${y1}" r="${sr}" fill="none"
      stroke="#4fc3f7" stroke-width="${(sw * 0.35).toFixed(1)}" stroke-opacity="0.9"/>
  </svg>`;
}
...
class ViewerHandler(SimpleHTTPRequestHandler):
    ...
    def _open_file(self):
        """Open the trajectory session directory in the system file manager."""
        import subprocess
        import sys as _sys
        ...
        try:
            if _sys.platform == "darwin":
                ...
                subprocess.Popen(["explorer", str(session_dir)])
            else:
