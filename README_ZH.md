...
自动复制配置文件、创建目录（`kb/`、`tests/`、`dataset/`）、安装依赖。
...
Distiller **仅使用像素坐标**操作设备（禁止选择器），将每次交互录制为 VLM 训练轨迹。它保留真实行为，包括误操作和纠正过程——为视觉推理模型提供真实训练信号。
...
| Skill | 职责 |
|-------|------|
| **uiautomator2** | 通过 `u2cli` 操作设备 — 点击、滑动、输入、截屏、应用管理 |
| **kb** | 读写持久化应用知识 — 页面、流程、选择器、异常行为 |
| **pytest** | 定义 pytest 脚本规范、fixture 约定、运行模式和 pause-on-failure 用法 |
