...
**探索一个页面：**
```text
@QAMule QA 探索设置应用首页，识别主要功能区，并总结当前可操作元素
```

**测试一个功能：**
```text
@QAMule QA 验证 Settings 中的蓝牙开关能否正常打开和关闭，并报告任何异常系统行为
```

**采集一条训练轨迹：**
```text
@QAMule Distiller 采集一条在 com.android.settings 中进入蓝牙设置页的真实设备交互轨迹
```
...
**查看已有测试覆盖：**
```text
/qamule:testcase 列出 Settings 当前已覆盖的功能点，并指出明显的覆盖空白
```

**预览数据集：**
```text
/qamule:dataset 预览最近采集的轨迹，并总结当前可用数据质量
```
...
默认恢复信号是 `SIGUSR1`，也可以通过 `--pause-resume-signal <SIGNAL>` 改成别的信号。

对于 shell 驱动的 Agent 运行，QAMule 提供了 `skills/pytest/scripts/monitor.sh` 用于监控日志，并提供 `skills/pytest/scripts/resume.sh` 用于恢复暂停的运行。

### Agent 检查点

QAMule 也支持在测试正常进行时主动插入 Agent checkpoint。测试可以在一个判断点暂停，把这个判断交给 Agent，然后继续执行。

`agent_checkpoint` 适合那些单靠本地断言不够稳的场景，例如判断过渡页面是否已可接受、视觉结果是否足够完整，或探索分支是否应算成功。

checkpoint 的约定保持得很简单：Agent 只返回布尔结果和可选原因，后续由测试自己决定如何继续。

对 Agent 驱动的 pytest 运行来说，整体流程仍然统一：把输出写入日志，让 `monitor.sh` 判断是否结束或暂停，再在需要时通过 `resume.sh` 继续。
...
<img src="./qa-agent-workflow-bilingual.excalidraw.svg" alt="QAMule QA Agent 工作流">
...
<img src="./distiller-agent-workflow-bilingual.excalidraw.svg" alt="QAMule Distiller 工作流">
...
| **pytest** | 内部 pytest 规范技能，定义脚本结构、fixture 约定、运行模式、pause-on-failure 和 Agent checkpoint 用法 |
